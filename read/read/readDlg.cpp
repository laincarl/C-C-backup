...
void CreadDlg::OnBnClickedBtnopen()
{
    ...
    if(dlg.DoModal  ())
    {
		...
		CString BmpName2 = _T("003.bmp");
		CString BmpName=dlg.GetPathName  ();             //得到路径名
		CopyFile(BmpName, BmpName2, TRUE);//将图片复制到当前目录下
        HBITMAP  hbitmap=(HBITMAP)LoadImage(AfxGetInstanceHandle(),BmpName,IMAGE_BITMAP,300,200,LR_LOADFROMFILE);  //装载位图   
		//显示图片1
...
//谷歌的方法
	...
	filter = "BMP文件 (*.bmp*)|*.bmp*||";//本题只处理bmp文件，故屏蔽其他类型图片文件
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		if (EntName.Compare(_T("bmp")) == 0)
		{
			...
			//显示图像1
			CWnd *pWnd = GetDlgItem(IDC_SHOWBEFORE); //获得pictrue控件窗口的句柄
			CRect rect;
			...
			//显示图像2
			CWnd *pWnd2 = GetDlgItem(IDC_SHOWAFTER); //获得pictrue控件窗口的句柄
			CRect rect2;
			pWnd2->GetClientRect(&rect2); //获得pictrue控件所在的矩形区域
			CDC *pDC2 = pWnd2->GetDC(); //获得pictrue控件的DC
			pDC2->SetStretchBltMode(COLORONCOLOR);
			StretchDIBits(pDC2->GetSafeHdc(), 0, 0, rect2.Width(), rect2.Height(), 0, 0,
				bmpInfo.biWidth, bmpInfo.biHeight, pBmpData, pBmpInfo, DIB_RGB_COLORS, SRCCOPY);
		}
	}*/
	//提示打开成功
