...
void CreadDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SMOOTH_HARD, m_group_smooth);
	DDX_Radio(pDX, IDC_SMOOTH_HARD, m_smooth);
}
...
BEGIN_MESSAGE_MAP(CreadDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_SHARP_WEAK, &CreadDlg::OnBnClickedSharpWeak)
END_MESSAGE_MAP()
...
void CreadDlg::Laplacian()
{
	...
	int nLength2 = m_edShow.SendMessage(WM_GETTEXTLENGTH);
	m_edShow.SetSel(nLength2, nLength2);//可以用来控制换行
	m_edShow.ReplaceSel(_T("x：") +y + _T("\n"));//以追加的方式增加文本
	for (int i = 1; i < height - 1; i++)
	{
		for (int j = 1; j < width - 1; j++)
		{
			temResult = (double)(*(pBmpData + (i - 1) * lineByte + j - 1) * temp[0]*x);
			temResult += (double)(*(pBmpData + (i - 1) * lineByte + j) * temp[1]*x);
			temResult += (double)(*(pBmpData + (i - 1) * lineByte + j + 1) * temp[2]*x);
			temResult += (double)(*(pBmpData + (i)* lineByte + j - 1) * temp[3]*x);
			temResult += (double)(*(pBmpData + (i)* lineByte + j) * (x*8+1));
			temResult += (double)(*(pBmpData + (i)* lineByte + j + 1) * temp[5]*x);
			temResult += (double)(*(pBmpData + (i + 1) * lineByte + j - 1) * temp[6]*x);
			temResult += (double)(*(pBmpData + (i + 1) * lineByte + j) * temp[7]*x);
			temResult += (double)(*(pBmpData + (i + 1) * lineByte + j + 1) * temp[8]*x);
			*(pNewBmpData + i * lineByte + j) = temResult*x;
		}
	}
}
...
void CreadDlg::OnBnClickedSharpWeak()
{
	m_sharp = 2;
	UpdateData(true);
	// TODO:  在此添加控件通知处理程序代码
